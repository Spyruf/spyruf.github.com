(function () {
    ...
    app.controller('UpNextController', ['$scope', '$filter', '$firebaseArray', '$firebaseObject', function ($scope, $filter, $firebaseArray, $firebaseObject) {
        ...
        $scope.myText = "";
        // this is necessary for button state stuff
        $scope.ab = {
            index: 0,
            isPlaying: false
        }
        ...
        $scope.init = function () {
            ...
            $scope.a.$bindTo($scope, 'ab').then(function () {
                ...
                if ($scope.ab.index == null || $scope.ab.isPlaying == null) {
            ...
            //--updates display when a queue or index is changed
            var unwatchQueue = $scope.queue.$watch(function () {
            ...
            //        var unwatchIndex = $scope.a.$watch(function () {
            //            console.log("data changed!");
            //            $scope.display = $filter('limitTo')($scope.queue, $scope.queue.length, $scope.ab.index + 1);
            //            $scope.artwork();
            //        });

            refI.on('child_changed', function (childSnapshot, prevChildKey) {
                $scope.display = $filter('limitTo')($scope.queue, $scope.queue.length, $scope.ab.index + 1);
                $scope.artwork(); // not working
